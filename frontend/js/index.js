            ...
            if (dataOnly) {
                ...
                while (n < x) {
                    ...
                    if (parseLinks) {
                        ...
                        if (printUser) {
                            var userStuff = '<div class="user">' + strip(authors[n].innerHTML) +
                                '</div>';
...
function handleTweets(tweets) {
    var x = tweets.length;
    var n = 0;
    var element = document.getElementById('theTweets');
    //var html = '<ul>';
    var html = '';
    while (n < x) {
        html += '<section class="posts">' + tweets[n] + '</div>';
        n++;
    }
    //html += '</ul>';
    element.innerHTML = html;
    setTimeout(function() {
        $("img[data-scribe='element:avatar']").each(function(index) {
